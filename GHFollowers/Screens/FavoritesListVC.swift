        ...
        PersistanceManager.retriveFavorites { [weak self] result in
            ...
            case .success(let favorites):
                if favorites.isEmpty{
                    self.showEmptyStateView(with: "Нет избранных пользователей. Добавьте их 😄.", in: self.view)
                } else {
...
extension FavoritesListVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        PersistanceManager.updateWith(favorite: favorite, actionType: .remove) { [weak self] error in
            ...
            self.presentGFAlertOnMainThread(title: "Не удалось удалить", message: error.rawValue, buttonTitle: "Ok")
        }
    }
}
